  ...
  reducers: {
    resetFilters(state) {
      return {
        ...
        bottomFilter: FilterAction.all,
        selectedFilters: [],
        unselectedFilters: state.activeFilter,
      };
    },
    setSelectedFilterList(state, action: PayloadAction<ISelectedFilter>) {
      const { filterType, filter } = action.payload;
      if (filterType === "add") {
        state.selectedFilters.push(filter);
        state.unselectedFilters = state.unselectedFilters.filter(
          ({ id }) => id !== filter.id
        );
        ...
        state.unselectedFilters.unshift(filter);
      }
    },
    setActiveFilters(state, action: PayloadAction<IFilters[]>) {
      state.activeFilter = action.payload;
      state.unselectedFilters = action.payload;
    },
    setFilters(state, action: PayloadAction<IFilters[]>) {
      if (action.payload) {
        state.filters = Array.from(
    ...
    setSelectedFilter(state, action: PayloadAction<ISelectedFilter>) {
      ...
      if (filterType === "add") {
        const newSelectedFilters = new Map(
          state.selectedFilters.map((f) => [f.id, f])
        );
        newSelectedFilters.set(filter.id, filter);
        return {
          ...state,
          selectedFilters: Array.from(newSelectedFilters.values()),
        };
      } else if (filterType === "remove") {
    ...
    setUnselectedFilters(state, action: PayloadAction<IUnselectedFilter>) {
      ...
      if (filterType === "updateAll") {
        ...
        const newUnselectedFilters = new Map(
          state.unselectedFilters.map((f) => [f.id, f])
        );
        filters.forEach((filter) =>
          newUnselectedFilters.set(filter.id, filter)
        );
        return {
          ...state,
          unselectedFilters: Array.from(newUnselectedFilters.values()),
        };
      } else if (filterType === "remove") {
        const unselectedFilterIds = new Set(filters.map((f) => f.id));
        const unselectedFilters = state.unselectedFilters.filter(
          ({ id }) => !unselectedFilterIds.has(id)
        );
        return { ...state, unselectedFilters };
      }
      return state;
