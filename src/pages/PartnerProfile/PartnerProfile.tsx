...
import { CalendarIcon, LocationIcon } from "../../components/Svg/Svg";
...
export default function PartnerProfile() {
  ...
  const { id: userId, avatar, partner, address } = user.selectedUser;
    ...
    (async function () {
      try {
        ...
        const withLiveOrder = events.notStarted.reduce(
          (acc, event) => {
            if (isBetweenDates(event.startDate, event.endDate))
              acc.live.push(event);
  ...
  const renderItem = useCallback(
    ({ item }: { item: IEventCart }) => {
      ...
      if (isEventActive) {
        ...
        return (
          <EventCardSmall
            event={{ ...el, user: user.selectedUser }}
            key={el.id}
          />
        );
      }
    ...
    <View style={[partnerProfileStyle.container]}>
        ...
        <View style={[partnerProfileStyle.partnerInfo]}>
          ...
          <View style={[partnerProfileStyle.addressWrapper]}>
            <CalendarIcon
              ...
              fill={"#64748B"}
            />
            <Text style={[partnerProfileStyle.addressName]}>
              {partnerEvents.notStarted.length} Events
            </Text>
          </View>
        </View>
      </View>
      <View style={[partnerProfileStyle.buttons]}>
        <ButtonStyled
          ...
          style={[
            partnerProfileStyle.buttonStyle,
