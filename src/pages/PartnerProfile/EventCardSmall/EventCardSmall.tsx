...
export default function EventCardSmall(props: EventCardSmall) {
  ...
  const { address } = useAppSelector((state) => state.user.selectedUser);
        ...
        <View style={[eventCardSmallStyle.dateContainer]}>
          <View style={[eventCardSmallStyle.dateWrapper]}>
            <CalendarIcon
              ...
              fill={"#64748B"}
            />
            <Text style={[eventCardSmallStyle.mainText]}>
              {dayjs(startDate).format("D MMMM, YY")}
            </Text>
          </View>
          <View style={[eventCardSmallStyle.dateWrapper]}>
            <ClockIcon fill={"#64748B"} width={13} height={14} />
            <Text style={[eventCardSmallStyle.mainText]}>
              {dayjs(startDate).format("ha")} - {dayjs(endDate).format("ha")}
            </Text>
          </View>
        </View>
